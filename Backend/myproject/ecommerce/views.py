...
class ContactoViewSet(viewsets.ModelViewSet):
    queryset = Contacto.objects.all()  # Devuelve todos los contactos en GET
    serializer_class = ContactoSerializer
    permission_classes = [IsAuthenticatedOrReadOnly]

    def perform_create(self, serializer):
        # Asocia el contacto al usuario autenticado
        serializer.save()

    def destroy(self, request, *args, **kwargs):
        contacto = request.data.get('id')

        if not contacto:
            return Response({'error': 'ID no proporcionado'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            contacto = Contacto.objects.get(nombre=contacto.nombre, email=contacto.email, asunto=contacto.asunto, mensaje=contacto.mensaje)
        except Contacto.DoesNotExist:
            return Response({'error': 'Contacto no encontrado'}, status=status.HTTP_404_NOT_FOUND)

        contacto.delete()
        return Response({'mensaje': 'Contacto eliminado correctamente'}, status=status.HTTP_204_NO_CONTENT)
